...
class AccountMove(models.Model):
    ...
    is_date_relance      = fields.Date(string='Date relance', help='Date dernière relance', readonly=1,tracking=True)
    is_date_releve       = fields.Date(string='Date relevé' , help='Date dernier relevé'  , readonly=1,tracking=True)
            ...
            for line in obj.line_ids:
                ...
                if internal_type in ('receivable', 'payable'):
                    for partial in line.matched_credit_ids:
                        ...
                        type_paiement = (dict(payment._fields['is_type_paiement'].selection).get(payment.is_type_paiement) or 'Règlement').lower()
                        num_cheque=''
                        ...
                        txt="Votre %s%s du %s"%(type_paiement,(num_cheque or ''),payment.date.strftime('%d/%m/%Y'))
